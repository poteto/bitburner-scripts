...
export async function main(ns) {
    ...
    const insufficientFunds = async (cost) => log(
        `Need: ${ns.nFormat(cost, '($0.00a)')}, have: ${ns.nFormat(await getHomeMoney(ns), '($0.00a)')}`,
        'warning'
    );
    ...
    log(`Successfully upgraded all hacknet nodes to level ${TARGET_LEVEL}`, 'success');
    ...
    log(`Successfully upgraded all hacknet nodes' ${TARGET_RAM}gb of RAM`, 'success');
    ...
    log(`Successfully upgraded all hacknet nodes to ${TARGET_CORES} cores`, 'success');
}
